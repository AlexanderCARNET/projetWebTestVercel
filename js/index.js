...
(() => {
  ...
  var webetu_url = "https://webetu.iutnc.univ-lorraine.fr";

  // lib/photoloader.ts
  function loadPicture(idPicture) {
    return __async(this, null, function* () {
      return fetch(`${api}/photos/${idPicture}`, { credentials: "include" }).then((response) => response.json().then((dataImage) => dataImage)).catch((error) => console.log(error));
    });
  }
  ...
  function displayPicture(dataImage) {
    var _a6;
    let html = (_a6 = document.querySelector("#photoTemplate")) == null ? void 0 : _a6.innerHTML;
    let template = import_handlebars.default.compile(html);
  ...
  function displayComments(commentaires) {
    let res = commentaires.reduce((acc, commentaire) => {
  ...
  function display_galerie(uri) {
    var _a6;
    let html = (_a6 = document.querySelector("#galerieTemplate")) == null ? void 0 : _a6.innerHTML;
    let template = import_handlebars2.default.compile(html);
  ...
  function load(uri) {
    return __async(this, null, function* () {
      let listeImgGallerie = [];
      let fetch2 = loadResource(uri);
      return fetch2.then((data) => {
  ...
  // lib/index.ts
  var URL_gallerie_origine = "/www/canals5/phox/api/photos";
  function getPicture(id) {
    let promise = loadPicture(id);
    promise.then((image) => __async(this, null, function* () {
      let dataImage = image.photo;
      dataImage.url.href = webetu_url + "/" + dataImage.url.href;
      displayPicture(dataImage);
      let cat = yield getCategorie(image.links.categorie.href);
      displayCategorie(cat);
      displayComments(yield getCommentaire(image.links.comments.href));
    }));
  }
  function getCategorie(uriImage) {
    return __async(this, null, function* () {
      return (yield loadResource(uriImage)).categorie.nom;
    });
  }
  function getCommentaire(uriComment) {
    return __async(this, null, function* () {
      return (yield loadResource(uriComment)).comments;
    });
  }
  var _a;
  (_a = document.querySelector("#load")) == null ? void 0 : _a.addEventListener("click", () => {
    display_galerie(URL_gallerie_origine);
  });
  var _a2;
  (_a2 = document.querySelector("#prev")) == null ? void 0 : _a2.addEventListener("click", () => {
    prev();
  });
  var _a3;
  (_a3 = document.querySelector("#next")) == null ? void 0 : _a3.addEventListener("click", () => {
    next();
  });
  var _a4;
  (_a4 = document.querySelector("#last")) == null ? void 0 : _a4.addEventListener("click", () => {
    last();
  });
  var _a5;
  (_a5 = document.querySelector("#first")) == null ? void 0 : _a5.addEventListener("click", () => {
    first();
  });
})();
